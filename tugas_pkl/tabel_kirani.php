...
function hitung($node = 0, $lvl = 0) { // Lengkapi sendiri parameternya

global $arr, $arr2;

// Jawaban ditulis di sini
$node_array = array("_id" => $node);
$getchild = getChildNodes($node_array);

if(!empty($node)){ $lvl = $arr[$node]['_level']; $arr2[$node] = $arr[$node];}

if(!empty($getchild)){
$lvl++; if($lvl >= 3){ asort($getchild); }
foreach($getchild as $parentId){
$arr2[$parentId] = $arr[$parentId];
$jumlah = hitung($parentId, $lvl);
if(!empty($node)){ $arr2[$node]['jumlah'] += $jumlah; }
}
}

if(!empty($node)){ return $arr2[$node]['jumlah']; }else{ return 0; }
}
...
// Proses Perhitungan
hitung(0);  // Lengkapi sendiri parameternya

// Tampilan tabel sesudah proses perhitungan
echo "<br><br><b>After: </b><br><table>";
